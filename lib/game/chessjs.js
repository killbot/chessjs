...
Chessjs = ig.Game.extend({
    ...
    loadSetup: function(whoIsViewing){
        ...
        this.currentSetup = GameModel;  //THIS is what you would change
        ...
        for (j=0; j<this.currentSetup.boardLayout.length; j++){          //y values
            for (i=0; i<this.currentSetup.boardLayout[j].length; i++){   //x values
                switch(this.currentSetup.boardLayout[j][i]){
                    case "bpawn":
                        ig.game.spawnEntity(EntityPawn, currBox.x, currBox.y, {color:"black"});
                        break;
                    case "wpawn":
                        ig.game.spawnEntity(EntityPawn, currBox.x, currBox.y, {color:"white"});
                        break
                    case "brook":
